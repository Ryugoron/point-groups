...
\end{lstlisting}
\todo{Muesste e nicht so erzeugt werden, dass$ l_i$ und $r_i$ innen in der Multiplikation sind? Oder beide aussen, aber nicht eins so, das andere so...}
...
Für vierdimensionale Gruppen mit Spiegelungen werden die Rotationsgruppen erweitert. Bei der Berechnung der Bilder werden dazu zunächst alle Bilder unter Rotationen bestimmt. Anschließend werden diese Bilder noch mit dem entsprechenden Quaternion \todo{wieder unklar, was erweitert heißt. Vielleicht oben die Funktion definieren und hier darauf verweisen?}erweitert. 
